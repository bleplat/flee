	...
	internal sealed class InternalXmlHelper {
		...
		public static object RemoveNamespaceAttributes(string[] inScopePrefixes, XNamespace[] inScopeNs, List<XAttribute> attributes, object obj) {
			if (obj is object) {
				...
				if (elem is object) return RemoveNamespaceAttributes(inScopePrefixes, inScopeNs, attributes, elem);
				else {
					IEnumerable elems = obj as IEnumerable;
					if (elems is object) return RemoveNamespaceAttributes(inScopePrefixes, inScopeNs, attributes, elems);
				}
			}
		...
		public static IEnumerable RemoveNamespaceAttributes(string[] inScopePrefixes, XNamespace[] inScopeNs, List<XAttribute> attributes, IEnumerable obj) {
			if (obj is object) {
				...
				if (elems is object) return elems.Select(new RemoveNamespaceAttributesClosure(inScopePrefixes, inScopeNs, attributes).ProcessXElement);
				else return obj.Cast<object>().Select(new RemoveNamespaceAttributesClosure(inScopePrefixes, inScopeNs, attributes).ProcessObject);
			}
		...
		private sealed class RemoveNamespaceAttributesClosure {
			...
			internal object ProcessObject(object obj) {
				XElement elem = obj as XElement;
				if (elem is object) return RemoveNamespaceAttributes(m_inScopePrefixes, m_inScopeNs, m_attributes, elem);
				else return obj;
			}
		}
							...
							if (inScopePrefixes is object && inScopeNs is object) {
								...
								for (int i = 0, loopTo = lastIndex; i <= loopTo; i++) {
									...
									if (prefix.Equals(currentInScopePrefix)) {
										if (ns == currentInScopeNs)                                             // prefix and namespace match.  Remove the unneeded ns attribute 
											a.Remove();
							...
							if (a is object) {
								...
								if (attributes is object) {
									...
									for (int i = 0, loopTo1 = lastIndex; i <= loopTo1; i++) {
										...
										if (currentInScopeNs is object) if (prefix.Equals(currentInScopePrefix)) {
												if (ns == currentInScopeNs)                                                     // prefix and namespace match.  Remove the unneeded ns attribute 
													a.Remove();
